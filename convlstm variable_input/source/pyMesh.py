...
def ellipticMap(x,y,h,tol):
    ...
    while True:
        X=(A*(x[2:,1:-1]+x[0:-2,1:-1])+C*(x[1:-1,2:]+x[1:-1,0:-2])-B/2*(x[2:,2:]+x[0:-2,0:-2]-x[2:,0:-2]-x[0:-2,2:]))/2/(A+C)
        Y=(A*(y[2:,1:-1]+y[0:-2,1:-1])+C*(y[1:-1,2:]+y[1:-1,0:-2])-B/2*(y[2:,2:]+y[0:-2,0:-2]-y[2:,0:-2]-y[0:-2,2:]))/2/(A+C)
        ...
        B=(x[2:,1:-1]-x[0:-2,1:-1])/2/h*(x[1:-1,2:]-x[1:-1,0:-2])/2/h+(y[2:,1:-1]-y[0:-2,1:-1])/2/h*(y[1:-1,2:]-y[1:-1,0:-2])/2/h+eps
...
def gen_e2vcg(x):
    ...
    for j in range(nelemy):
        for i in range(nelemx):
            e2vcg[:,j*nelemx+i] = np.asarray([j*nnx+i,j*nnx+i+1,(j+1)*nnx+i,(j+1)*nnx+i+1])
...
class hcubeMesh(object):
    def __init__(self, T0, leftX, leftY, rightX, rightY, lowX, lowY, upX, upY, h,
