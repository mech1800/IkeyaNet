...
def dfdt(f,i):
    # 2次精度の右側片側差分
    if i == 0 or i == 1:
        dfdt = (-3*f[:,i,:,:,:]+4*f[:,i+1,:,:,:]-f[:,i+2,:,:,:])/(2*time_step)
    # 2次精度の左側片側差分
    elif i == sequence-2 or i == sequence-1:
        dfdt = (3*f[:,i,:,:,:]-4*f[:,i-1,:,:,:]+f[:,i-2,:,:,:])/(2*time_step)
    # 2次精度の中心差分
    else:
        dfdt = (f[:,i+1,:,:,:]-f[:,i-1,:,:,:])/(2*time_step)
    return dfdt
...
def train(epoch, outputV):
    ...
    for iteration, batch in enumerate(training_data_loader):
        ...
        for i in range(sequence):
            ...
            # 2次精度の中心差分 i==0
            if i == 0:
                dvdt_i = (outputV[:,i,:,:,:]-T0)/(2*time_step)
            # 2次精度の左側片側差分 i==-1
            elif i == sequence-1:
                dvdt_i = (3*outputV[:,i,:,:,:]-4*outputV[:,i-1,:,:,:]+outputV[:,i-2,:,:,:])/(2*time_step)
            # 2次精度の中心差分
            else:
                dvdt_i = (outputV[:,i+1,:,:,:]-outputV[:,i-1,:,:,:])/(2*time_step)
            '''
            ...
            # 3次精度の右側片側差分
            if i == 0:
                dvdt_i = (-11*outputV[:,i,:,:,:]+18*outputV[:,i+1,:,:,:]-9*outputV[:,i+2,:,:,:]+2*outputV[:,i+3,:,:,:])/(6*time_step)
            # 4次精度の中心差分 i==1
            elif i == 1:
                dvdt_i = (-outputV[:,i+2,:,:,:]+8*outputV[:,i+1,:,:,:]-8*outputV[:,i-1,:,:,:]+T0)/(12*time_step)
            # 3次差分の左側片側差分
            elif i == sequence-1 or i == sequence-2:
                dvdt_i = (11*outputV[:,i,:,:,:]-18*outputV[:,i-1,:,:,:]+9*outputV[:,i-2,:,:,:]-2*outputV[:,i-3,:,:,:])/(6*time_step)
            # 4次精度の中心差分
            else:
                dvdt_i = (-outputV[:,i+2,:,:,:]+8*outputV[:,i+1,:,:,:]-8*outputV[:,i-1,:,:,:]+outputV[:,i-2,:,:,:])/(12*time_step)
            '''
    ...
    if epoch%nEpochs==0:
        ...
        # ①横軸(0~10),縦軸(予測したTとOpenFOAMのエラー)のグラフを作る
